...
<html lang="id">
    <head>
        ...
        <title>Daftar sebagai calon mahasiswa — {{ config('app.name') }}</title>
    ...
    <body class="bg-porcelain-50 font-sans text-ink-900 antialiased dark:bg-ink-950 dark:text-porcelain-100">
        <div class="flex min-h-[100dvh] items-center justify-center px-4 py-6 sm:px-6 lg:px-8 lg:py-6">
            <div class="w-full max-w-5xl">
                ...
                {{-- Kembaran halaman masuk (auth/login.blade.php), foto dan formulir
                     ditukar sisi: foto di kiri, formulir di kanan — supaya kedua
                     layar terasa satu pasang, bukan dua desain yang kebetulan mirip. --}}
                <div class="grid grid-cols-1 overflow-hidden rounded-[2rem] border border-black/5 bg-white shadow-2xl shadow-ink-950/10 dark:border-white/10 dark:bg-ink-900/60 lg:grid-cols-2">
                    ...
                    <div class="animate-fade-up p-8 sm:p-10 lg:order-2 lg:px-10 lg:py-6">
                        <span class="inline-flex items-center gap-2 rounded-full border border-black/10 bg-black/[0.03] px-3 py-1 font-mono text-[11px] uppercase tracking-[0.2em] text-ink-500 dark:border-white/10 dark:bg-white/5 dark:text-porcelain-300/70">
                            <x-heroicon-o-user-plus class="h-3.5 w-3.5 text-brand-600" aria-hidden="true" />
                        ...
                        <h1 class="mt-4 font-display text-4xl font-extrabold uppercase leading-[1.05] tracking-tight text-ink-950 dark:text-porcelain-50">
                            Buat akun<br>baru Anda.
                        </h1>

                        <p class="mt-3 max-w-sm text-sm leading-relaxed text-ink-600 dark:text-porcelain-300/70">
                            Gratis, dan hasil tes rekomendasi program studi Anda tersimpan untuk dibuka kembali kapan saja.
                        </p>

                        <form method="POST" action="{{ route('register') }}"
                              x-data="{ submitting: false, password: '', confirmation: '' }" @submit="submitting = true"
                              class="mt-5 space-y-3">
                            @csrf

                            {{-- Nama, surel, dan kata sandi digambar sebagai satu
                                 "jalur" — titik-titik terhubung garis putus-putus —
                                 kembaran pola field pada halaman masuk. --}}
                            <div class="relative rounded-2xl border border-black/10 bg-black/[0.02] px-4 py-1 dark:border-white/10 dark:bg-white/5">
                                ...
                                <div class="relative flex items-center gap-3 py-2">
                                    <span class="z-10 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-black/10 bg-white dark:border-white/10 dark:bg-ink-900">
                                        <x-heroicon-o-user class="h-3.5 w-3.5 text-ink-500 dark:text-porcelain-300" />
                                    </span>
                                    <input id="name" name="name" type="text" required autofocus autocomplete="name"
                                           value="{{ old('name') }}" placeholder="Nama lengkap sesuai ijazah"
                                ...
                                <div class="relative flex items-center gap-3 py-2" x-data="{ visible: false }">
                                    ...
                                    <input id="password" name="password" required autocomplete="new-password"
                                           placeholder="Kata sandi (minimal 8 karakter)" x-model="password"
                                           x-bind:type="visible ? 'text' : 'password'" type="password"
                                           class="w-full border-0 bg-transparent p-0 text-sm text-ink-900 placeholder:text-ink-400 focus:outline-none focus:ring-0 dark:text-porcelain-100 dark:placeholder:text-porcelain-400/50">
                                ...
                                <div class="relative flex items-center gap-3 py-2">
                                    ...
                                    <input id="password_confirmation" name="password_confirmation" type="password" required
                                           autocomplete="new-password" placeholder="Ulangi kata sandi" x-model="confirmation"
                                           class="w-full border-0 bg-transparent p-0 text-sm text-ink-900 placeholder:text-ink-400 focus:outline-none focus:ring-0 dark:text-porcelain-100 dark:placeholder:text-porcelain-400/50">
                                </div>
                            </div>
                            <x-input-error :messages="array_merge($errors->get('name'), $errors->get('email'), $errors->get('password'), $errors->get('password_confirmation'))" class="mt-2" />

                            {{-- Umpan balik langsung, bukan cuma menunggu validasi server —
                                 supaya kesalahan ketik ketahuan sebelum tombol ditekan. --}}
                            <p class="flex items-center gap-1.5 text-xs"
                               x-show="confirmation.length > 0" x-cloak
                               :class="password === confirmation ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'">
                                <x-heroicon-o-check-circle class="h-3.5 w-3.5" x-show="password === confirmation" />
                                <x-heroicon-o-x-circle class="h-3.5 w-3.5" x-show="password !== confirmation" x-cloak />
                                <span x-text="password === confirmation ? 'Kata sandi cocok' : 'Kata sandi belum sama'"></span>
                            </p>
                            ...
                            <div class="flex flex-wrap items-center gap-5 pt-2">
                                <button type="submit" :disabled="submitting"
                                    ...
                                    <span x-text="submitting ? 'Memproses…' : 'Buat Akun'"></span>
                                </button>
                        ...
                        <p class="mt-4 flex items-start gap-2.5 rounded-lg border border-black/5 bg-black/[0.02] px-4 py-2.5 text-xs leading-relaxed text-ink-500 dark:border-white/10 dark:bg-white/5 dark:text-porcelain-300/60">
                            <x-heroicon-o-information-circle class="mt-0.5 h-4 w-4 shrink-0 text-ink-400 dark:text-porcelain-400/60" />
                            <span>
