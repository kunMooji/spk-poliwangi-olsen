<x-guest-layout title="Masuk ke akun Anda"
            ...
            <div class="flex items-center justify-between">
                ...
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}"
                       class="text-xs font-medium text-brand-600 transition hover:text-brand-500 dark:text-brand-400">
                        Lupa kata sandi?
                    </a>
                @endif
            </div>
    ...
    <p class="mt-6 border-t border-gray-100 pt-6 text-center text-sm text-gray-600 dark:border-gray-700 dark:text-gray-400">
        Belum punya akun?
        <a href="{{ route('register') }}" class="font-semibold text-brand-600 transition hover:text-brand-500 dark:text-brand-400">
